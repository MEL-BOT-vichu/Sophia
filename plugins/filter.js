...
Asena.addCommand({on: 'text', fromMe: false}, (async (message, match) => {
        ...
        let banned = jid.find( Jid => Jid === message.jid);
        if(banned !== undefined) return
        if (!!message.mention && message.mention[0] == '916282715084@s.whatsapp.net') {
await message.client.sendMessage(message.jid, fs.readFileSync('./uploads/lol.mp3'), MessageType.audio, { mimetype: Mimetype.mp4Audio, quoted : message.data, ptt: true})
        }
const array = ['1','2','AMUL','Aarulle','Adarsh','Adipoli','Ambadi','Amul','BOT','Back','Bad','Bgm','Bot','Bye','Chiri','Chunk','Chunks','Cristiano','Cry','Dai','Eda','Ee','Fek','Fen','Frnd','Ha','Hello','Hi','Hlo','Hy','I am back','I love you','Kali','Kalikan nikale','Kgf','Kk','Kukku','MEL-BOT','Machan','Mad','Malang','Mass','Mathy','Meeting','Mel','Men','Mohanlal','Muscle','Myre','Nanni','Njan','Onnu poda','Panni','Place','Poda myre','Podai','Potta','Ramos','Rashmika','SUREASH','SURESH','Sed','Senior','Shey','Sry','Sureash','Suresh','Thalapathy','Tholvi','Thyr','Tommy Shelby','Uyr','VICHU','Vichu','Yaar','aara','adarsh','adi','adima','alive','amul','ara','aysheri','baby','bei','bie','bot','bye','care','chatho','comedy','da','don','ekk','enjoy','enth','fd','fresh','gd n8','gdmng','goat','grp','hate','help','king','konami','kozhi','kundan','kunna','life','lol','love u','malar','mel','messi','mier','mood','mukesh','muthe','myr','myre','nanban','neymar','pettu','pewer','return','ronaldo','saji','sed','set aano','shey','sulthan','suresh','sureash','theri','thot','umbi','vanam','vanno','wow']
array.map( async (a) => {
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
       await message.client.sendMessage(message.jid, fs.readFileSync('./uploads/' + a + '.mp3'), MessageType.audio, { mimetype: Mimetype.mp4Audio, quoted: message.data, ptt: true})
...
Asena.addCommand({on: 'text', fromMe: false}, (async (message, match) => {
    ...
    if(banned !== undefined) return
    if (!!message.mention && message.mention[0] == '916282715084@s.whatsapp.net') {
await message.client.sendMessage(message.jid, fs.readFileSync('./sticker/TOMMY SHELBY.webp'), MessageType.sticker, { mimetype: Mimetype.webp, quoted : message.data, ptt: false})
    }
const array = ['Pikachu','Msd','Vijay','Rashmika','song','Sry','Line','Aarulle','achodaa','ayin','TOMMY SHELBY','Aysheri','Ayye','Ayyo','broken','bye','chattho','cute','Da','Eee','Eee2','engane und','Entha','Enthada','Girls','Good morning','Good night','Hi','Hy','ithokke enth','ivan','Kurippe','Kurumb','Love','Mm','naanam','nadakkatte','Ok','paavam','Pattumo','pikachu','Pm','poda','Pova','Save','setth','sho','Shoo','Smile','tag','Udayipp','umma','Vaa','Vannu','yo','ys' ]
array.map( async (a) => {
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
