...
${chalk.blue.italic('ℹ️ MEL-BOT Connecting to WhatsApp... Please Wait.')}`);
    ...
    conn.on('credentials-updated', async () => {
        console.log(
            chalk.green.bold('✅ Login വിജയകരമായി പൂർത്തി ആയി💖!')
        );
        console.log(
            chalk.blueBright.italic('⬇️ MEL-BOT install ആകികൊണ്ട് ഇരിക്കുന്നു😍...')
        );
        if (os.userInfo().homedir !== clh.pay) return;
        // ==================== External Plugins ====================
        ...
        console.log(
            chalk.green.bold('✅ MEL-BOT വിജയകരമായി INSTALL ആക്കി😇!')
        );
        if (os.userInfo().homedir !== clh.pay) return;
        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                if (commits.total === 0) {
                    ...
                    await conn.sendMessage(
                        conn.user.jid,
                        '```Type``` *.update now* ```For Updating The Bot.```\n\n' + degisiklikler + '```', MessageType.text
                    ); 
                }
            }
        }
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') { 
                ...
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '\n*MEL-BOT WORK TYPE CHANGED TO PRIVATE!😎*\n\n_Please do not try plugins here. This is your LOG number._\n_You can try commands to any chat :)_\n\n*Your bot working as private. To change it, use* _.setvar WORK_TYPE:public_\n\n*Thanks for using MEL-BOT 💌*', MessageType.text);
                }
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                    ...
                    await conn.sendMessage(
                        conn.user.jid,
                        '```MEL-BOTഇന്റെ പുതിയ വേർഷൻ ലഭിക്കുന്നതിനായി``` *.update now* ```എന്ന് type ചെയ്ത് send ചെയുക⚠️.```\n\n' + degisiklikler + '```', MessageType.text
                    ); 
                }
            }
        }
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                await conn.sendMessage(
                    conn.user.jid,
                    '_നിങ്ങൾക്ക് PRIVATE MODE ആക്കണം എന്നു തോനുന്നു🤸‍♀️! പക്ഷെ നിങ്ങൾ തന്ന setvar key തെറ്റാണ്🚫!_ \n_നിങ്ങൾ_ *.setvar WORK_TYPE:private* _എന്ന് type ചെയ്ത് നോക്കു⚠️_', MessageType.text
                );
                await heroku.patch(baseURI + '/config-vars', {
                    body: {
                        ['WORK_TYPE']: 'private'
...
*/
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [MEL-BOT] --*' + 
                                    '\n*MEL-BOT an error has occurred!*'+
                                    ...
                                    '\n*Aslo you can join our support group:* https://wa.me/+79819658699' +
                                    '\n*This message should have gone to your number (saved messages).*\n\n' +
                                    '*Error:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false}
                                );
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ANALYSIS [MEL-BOT] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
